...
dir('./data2')
list.files('./data2')  # data files we created
...
head(iris)
?iris
write.csv(iris,"./data/iris.csv", row.names=F) # don't want rows in CSV file
...
library(xlsx)
library(rJava)
sys.setenv(JAVA-HOME='C:\\Program Files\\Java\\jre')
...
plot(x=women$weight, y=women$height, pch=15, xlab='Weight', ylab='Height', col='red', cex=2, type='b')  #CEX means making it big
